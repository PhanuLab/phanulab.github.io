...
# Invalidate the files on CloudFront:
# - get the CloudFront distribution from private repository
cloudFrontDistributionId=$(getCloudFrontDistribution)
if [ -z "${cloudFrontDistributionId}" ]; then
  echo "Unable to get the CloudFront distribution."
  exit 1
fi
cloudFrontFolder="/owf-learn-mkdocs/*"
